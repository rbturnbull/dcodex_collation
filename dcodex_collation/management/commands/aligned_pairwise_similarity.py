...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "siglum1", type=str, help="The siglum the first manuscript."
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            const=True,
            default=False,
            help="Restricts output to only sections where there enough alignments to fill the window.",
        )
        parser.add_argument(
            "--all-transitions",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        if window % 2 == 0:
            raise ValueError(
                f"Window value of {window} is not allowed. It must be odd."
            )
        ...
        if options["all_transitions"] == False:
            for to_ignore in TransitionTypeToIgnore.objects.all():
                for x in range(10):
                    for transition in Transition.objects.filter(
                        column__alignment__verse__id__in=intersection_verse_ids,
                        transition_type=to_ignore.transition_type,
                    ):
                        ...
                        np.place(
                            states_ms2,
                            states_ms2 == max(start_state_id, end_state_id),
                            min(start_state_id, end_state_id),
                        )
                        np.place(
                            states_ms1,
                            states_ms1 == max(start_state_id, end_state_id),
                            min(start_state_id, end_state_id),
                        )
        ...
        column_counts = np.array(
            [
                Column.objects.filter(alignment__verse__id=verse_id).count()
        ...
        for verse_id in intersection_verse_ids_array:
            indexes_in_window = (
                intersection_verse_ids_array >= verse_id - half_window
            ) & (intersection_verse_ids_array <= verse_id + half_window)
            ...
            agreement_count = np.sum(
                agreements[
                    (verse_ids_per_cell >= verse_id - half_window)
                    & (verse_ids_per_cell <= verse_id + half_window)
                ]
            )
        ...
        verses = [
            verse_class.objects.get(id=verse_id)
            for verse_id in intersection_verse_ids_array
        ]
        ...
        xtick_verses = [
            verse_class.objects.filter(rank=rank).first() for rank in xticks
        ]
        tick_text = [
            verse.reference(abbreviation=True) if verse else ""
            for verse in xtick_verses
        ]
