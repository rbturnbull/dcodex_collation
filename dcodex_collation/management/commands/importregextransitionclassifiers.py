...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "CSV",
            type=str,
            help="A CSV file with the start state regex, end state regex and the transition type.",
        )
    ...
    def handle(self, *args, **options):
        ...
        for index, row in df.iterrows():
            ...
            transition_type = TransitionType.objects.filter(
                name=row["transition_type"]
            ).first()
            start_state_regex, end_state_regex = (
                row["start_state_regex"],
                row["end_state_regex"],
            )
            if transition_type == None:
                transition_type = TransitionType.objects.filter(
                    inverse_name=row["transition_type"]
                ).first()
            ...
            if "name" in row:
                ...
                classifier_name = (
                    f"{start_state_regex} -> {end_state_regex}: {transition_type.name}"
                )
