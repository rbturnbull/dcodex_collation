...
class AlignmentFromTokensTest(TestCase):
    def setUp(self):
        ...
        self.alignment, _ = Alignment.objects.update_or_create(
            family=family, verse=verse
        )
        ...
        for row_index in range(10):
            ...
            row, _ = Row.objects.update_or_create(
                alignment=self.alignment, transcription=transcription
            )
    ...
    def test_add_column(self):
        ...
        self.assertEqual(
            self.alignment.column_set.count(), self.original_columns_count + 1
        )
    ...
    def check_shift(self, delta, initial_col_order):
        ...
        column = Column.objects.get(
            id=column.id
        )  # Get the column again from the database
        ...
        new_column = Column.objects.get(
            order=final_column_order + delta, alignment=self.alignment
        )
...
class AlignmentFromTranscriptionsTest(TestCase):
    ...
    def test_update_transcription_in_alignment_longer(self):
        ...
        # Update alignment
        new_alignment = update_transcription_in_alignment(
            transcription, self.gotoh_param, gap_open=-2, gap_extend=-1
        )
...
class RegexTransitionClassifierTest(TestCase):
    ...
    def test_regex_match(self):
        self.assertEqual(
            self.classifier.match(self.column, self.state1, self.state2), True
        )
        self.assertEqual(
            self.classifier.match(self.column, self.state2, self.state1), False
        )
